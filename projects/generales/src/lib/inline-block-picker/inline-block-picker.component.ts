...
export class InlineBlockPickerComponent implements OnInit, InlineBlockerPickerConfig {
  ...
  constructor(private dialog: MatDialog, @Inject(MAT_DIALOG_DATA) public data: { maxItems: number; chipsInput: Array<InlineBlockPicker> }) {
  ...
  public emitirClick(objetoSeleccionado: InlineBlockPicker, ...args) {
    ...
    this.chipSelection.emit({
      ...
      valor: this._chipsInput.filter((entry) => entry.selected).map((entry) => entry.texto),
  ...
  public cambioPaginaPaginador(event: PageEvent) {
    ...
    this.sliced_chipsInput = this._chipsInput.slice(punteroInicial, punteroMAximoArray);
  }

  public returnArrayListChips() {
    return this.sliced_chipsInput && this.sliced_chipsInput.length > 0 ? this.sliced_chipsInput : this._chipsInput;
  }
}
